...
import type { DiagnosticInfo } from "@claude-vscode/shared";
...
export function registerTools(server: McpServer, client: BridgeClient): void {
  server.registerTool(
    ...
    {
      description: "List files currently open in the VSCode editor, with dirty/active state.",
      inputSchema: {},
    },
  ...
  server.registerTool(
    ...
    {
      description: "Get the file currently focused in the VSCode editor, or null if none.",
      inputSchema: {},
    },
    async () => textResult(await client.call("editor/getActiveEditor", undefined)),
  );

  server.registerTool(
    "vscode_get_selection",
    {
      description: "Get the current text selection(s) in a file. Uses the active editor if no path is given.",
      inputSchema: { path: z.string().optional().describe("Absolute path of the file. Defaults to the active editor.") },
    },
    async ({ path }) => textResult(await client.call("editor/getSelection", { path })),
  );
  ...
  server.registerTool(
    "vscode_get_diagnostics",
    {
      ...
      inputSchema: { path: z.string().optional().describe("Absolute path of the file. Omit for all open diagnostics.") },
    },
    async ({ path }) => formatDiagnostics(await client.call("editor/getDiagnostics", { path })),
  );

  server.registerTool(
    "vscode_get_workspace_symbols",
    {
      description: "Search workspace-wide symbols (functions, classes, etc.) by name.",
      inputSchema: { query: z.string().describe("Symbol name or fuzzy query") },
    },
    async ({ query }) => textResult(await client.call("editor/getWorkspaceSymbols", { query })),
  );

  server.registerTool(
    "vscode_get_file_content",
    {
      description: "Read the current on-disk content of a file via VSCode's filesystem layer.",
      inputSchema: { path: z.string().describe("Absolute path of the file") },
    },
    async ({ path }) => textResult(await client.call("editor/getFileContent", { path })),
  );
  ...
  server.registerTool(
    "vscode_apply_edit",
    {
      description: "Apply one or more text edits to a file through VSCode's editor (supports undo, respects unsaved state).",
      inputSchema: {
  ...
  server.registerTool(
    ...
    {
      description: "Open a file and move the cursor/selection to a specific position.",
      inputSchema: {
  ...
  server.registerTool(
    "vscode_get_tasks",
    {
      description: "List tasks defined in the workspace (from tasks.json and task providers).",
      inputSchema: {},
    },
  ...
  server.registerTool(
    "vscode_debug_start",
    {
      ...
      inputSchema: {
        configName: z.string().optional().describe("Name of the launch configuration. Uses the default if omitted."),
  ...
  server.registerTool(
    ...
    {
      description: "Get captured output from a terminal that was run via vscode_terminal_run.",
      inputSchema: {
        name: z.string().optional().describe("Terminal name. Defaults to the active terminal."),
        maxLines: z.number().int().positive().optional().describe("Only return the last N lines."),
      },
    },
    async ({ name, maxLines }) => textResult(await client.call("terminal/getOutput", { name, maxLines })),
  );
  ...
  server.registerTool(
    "vscode_show_message",
    {
      description: "Show a VSCode notification, optionally with action buttons, and return which one was clicked.",
      inputSchema: {
        message: z.string(),
  ...
  server.registerTool(
    ...
    {
      description: "Show a VSCode quick-pick list and return the selected item.",
      inputSchema: {
